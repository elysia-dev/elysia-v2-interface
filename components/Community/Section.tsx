import { Trans, useTranslation } from 'react-i18next';
import { SectionWrapper } from './styles';
import Image from 'next/image';
import Discord from 'assets/images/community/discord_white@2x.webp';
import Git from 'assets/images/community/git@2x.webp';
import Facebook from 'assets/images/community/facebook@2x.webp';
import Kakao from 'assets/images/community/kakao_white@2x.webp';
import Medium from 'assets/images/community/medium@2x.webp';
import Telegram from 'assets/images/community/telegram_white@2x.webp';
import Twitter from 'assets/images/community/twitter@2x.webp';
import Wechat from 'assets/images/community/wechat@2x.webp';
import useIsMobile, { MediaQueryState } from 'hooks/useIsMobile';
import ContentItem from 'components/Common/ContentItem';
import useResizeBrowser from 'hooks/useResizeBrowser';
import PageHeader from 'components/Common/PageHeader';
import { GoogleAnalyticsEvent } from 'utils/gaEvent';
import GoogleAnalyticsCategory from 'enums/GoogleAnalyticsCategory';
import GoogleAnalyticsAction from 'enums/GoogleAnalyticsAction';

const Section = () => {
  const { t } = useTranslation();
  const { mediaQueryState } = useIsMobile();
  const { browserHeight } = useResizeBrowser();

  return (
    <>
      <SectionWrapper theme={browserHeight}>
        <PageHeader
          headers={[t('community.0'), t('community.1'), t('community.2')]}
        />
        <article>
          <h2>{t('community.3')}</h2>
          <div>
            <ContentItem
              Icon={
                <Image
                  src={Discord}
                  alt={'Discord'}
                  width={mediaQueryState === MediaQueryState.Desktop ? 20 : 30}
                  height={mediaQueryState === MediaQueryState.Desktop ? 20 : 30}
                />
              }
              contentName={'Discord'}
              link={'https://discord.gg/d8zveNum7gg'}
              onClickEvent={() =>
                GoogleAnalyticsEvent(
                  GoogleAnalyticsAction.CommunityDiscord,
                  GoogleAnalyticsCategory.Community,
                )
              }
            />
            <ContentItem
              Icon={
                <Image
                  src={Telegram}
                  alt={'Telegram'}
                  width={mediaQueryState === MediaQueryState.Desktop ? 21 : 31}
                  height={mediaQueryState === MediaQueryState.Desktop ? 21 : 31}
                />
              }
              contentName={'Telegram'}
              link={'https://t.me/elysia_official'}
              onClickEvent={() =>
                GoogleAnalyticsEvent(
                  GoogleAnalyticsAction.CommunityTelegram,
                  GoogleAnalyticsCategory.Community,
                )
              }
            />
            <ContentItem
              Icon={
                <Image
                  src={Kakao}
                  alt={'Kakao'}
                  width={mediaQueryState === MediaQueryState.Desktop ? 19 : 29}
                  height={mediaQueryState === MediaQueryState.Desktop ? 19 : 29}
                />
              }
              contentName={'Kakaotalk'}
              link={'https://open.kakao.com/o/gUpSOwkb'}
              onClickEvent={() =>
                GoogleAnalyticsEvent(
                  GoogleAnalyticsAction.CommunityKakaotalk,
                  GoogleAnalyticsCategory.Community,
                )
              }
            />
            <div></div>
          </div>
        </article>
        <article>
          <h2>{t('community.5')}</h2>
          <div>
            <ContentItem
              Icon={
                <Image
                  src={Git}
                  alt={'Git'}
                  width={
                    mediaQueryState === MediaQueryState.Desktop ? 24.42 : 34.42
                  }
                  height={
                    mediaQueryState === MediaQueryState.Desktop ? 24.42 : 34.42
                  }
                />
              }
              contentName={'Github'}
              link={'https://github.com/elysia-dev'}
              onClickEvent={() =>
                GoogleAnalyticsEvent(
                  GoogleAnalyticsAction.CommunityGithub,
                  GoogleAnalyticsCategory.Community,
                )
              }
            />
            <ContentItem
              Icon={
                <Image
                  src={Medium}
                  alt={'Tech Blog'}
                  width={mediaQueryState === MediaQueryState.Desktop ? 28 : 38}
                  height={mediaQueryState === MediaQueryState.Desktop ? 11 : 21}
                />
              }
              contentName={'Tech Blog'}
              link={'https://tech.elysia.land/'}
              onClickEvent={() =>
                GoogleAnalyticsEvent(
                  GoogleAnalyticsAction.CommunityTechBlog,
                  GoogleAnalyticsCategory.Community,
                )
              }
            />
            <div></div>
            <div></div>
          </div>
        </article>
        <article>
          <h2>{t('community.6')}</h2>
          <div>
            <ContentItem
              Icon={
                <Image
                  src={Twitter}
                  alt={'Twitter'}
                  width={mediaQueryState === MediaQueryState.Desktop ? 21 : 31}
                  height={mediaQueryState === MediaQueryState.Desktop ? 21 : 31}
                />
              }
              contentName={'Twitter'}
              link={'https://twitter.com/Elysia_HQ'}
              onClickEvent={() =>
                GoogleAnalyticsEvent(
                  GoogleAnalyticsAction.CommunityTwitter,
                  GoogleAnalyticsCategory.Community,
                )
              }
            />
            <ContentItem
              Icon={
                <Image
                  src={Facebook}
                  alt={'Facebook'}
                  width={
                    mediaQueryState === MediaQueryState.Desktop ? 23.52 : 33.52
                  }
                  height={
                    mediaQueryState === MediaQueryState.Desktop ? 23.52 : 33.52
                  }
                />
              }
              contentName={'Facebook'}
              link={'https://www.facebook.com/ElysiaHQ'}
              onClickEvent={() =>
                GoogleAnalyticsEvent(
                  GoogleAnalyticsAction.CommunityFacebook,
                  GoogleAnalyticsCategory.Community,
                )
              }
            />
            <ContentItem
              Icon={
                <Image
                  src={Medium}
                  alt={'Medium'}
                  width={mediaQueryState === MediaQueryState.Desktop ? 28 : 38}
                  height={mediaQueryState === MediaQueryState.Desktop ? 11 : 21}
                />
              }
              contentName={'Medium'}
              link={'https://medium.com/elysia-magazine'}
              onClickEvent={() =>
                GoogleAnalyticsEvent(
                  GoogleAnalyticsAction.CommunityMedium,
                  GoogleAnalyticsCategory.Community,
                )
              }
            />
            <ContentItem
              Icon={
                <Image
                  src={Wechat}
                  alt={'Weibo'}
                  width={mediaQueryState === MediaQueryState.Desktop ? 22 : 32}
                  height={mediaQueryState === MediaQueryState.Desktop ? 16 : 26}
                />
              }
              contentName={'Weibo'}
              link={'https://weibo.com/u/7449962304'}
              onClickEvent={() =>
                GoogleAnalyticsEvent(
                  GoogleAnalyticsAction.CommunityWeibo,
                  GoogleAnalyticsCategory.Community,
                )
              }
            />
          </div>
        </article>
      </SectionWrapper>
    </>
  );
};

export default Section;
